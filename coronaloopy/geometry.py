...
def semi_circular_loop(length: u.cm,
                       ...
                       gamma: u.deg = 0*u.deg):
    """
    Generate world coordinates for a semi-circular loop who's footpoints
    are rooted on the surface of the Sun.
...
def semi_circular_arcade(length: u.cm, width: u.deg, num_strands, observer, **kwargs):
    """
    Generate an arcade of `num_loops` of full length `length` evenly spaced
